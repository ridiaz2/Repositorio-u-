...
class SistemaElectrico:
            ...
            if vertex not in visited:
                if vertex.nodo != "G":
                    ...
                    for num in range_(len(potencias_entregar)):
                        ...
                        nodo_num.potencia = \
                            max(valor_mij - (valor_mij * resistividad * float(
                                vertex.children.descomprimir(1)[num]) /
                                nodo_num.seccion_transversal), 0)
                        nodo_num.mij, nodo_num.mij_0 = valor_mij, nodo_num.mij
    ...
    def agregar_conexion(self, desde, hasta, largo, confirmado=False):
        ...
        hasta.hasta.agregar(Tupla(desde, largo))
    ...
    def mayor_menor_consumo(self, sistema, tipo=0):
        ...
        if int(tipo) == 1:
            nodo = max(self.casas.valores(), key=lambda i: float(i.consumo))
            accion = "mayor"
        elif int(tipo) == 2:
            accion = "menor"
            nodo = min(self.casas.valores(), key=lambda i: float(i.consumo))

        return Tupla(accion, nodo.id, nodo.provincia, nodo.comuna)
