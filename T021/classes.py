...
class Casino:
    ...
    def go_actividad(self, cliente):
        ...
        if cliente.personalidad == "kitbitzer" and cliente.hablado:
    ...
    def mover_cliente(self, cliente, camino):
        ...
        if cliente.accion == "retirar" and not camino:
            cliente.accion = ""
            cliente.deleteLater()
            cliente.transportandoce = False
            self.clientes.pop(cliente.id)
        elif not camino:
            cliente.transportandose = False
            if not cliente.tiempo_accion:
                if cliente.lugar.tipo in ("tragamonedas", "ruleta"):
...
class Cliente(Individuo, Human):
    ...
    def siguiente_accion(self):
        return choice(["jugar", "instalacion"])
...
class Ruleta(Juego, Game):
    ...
    def prob_ruleta(self, probabilidad, predecir=False):
        return probabilidad + ((par.q / 100) * probabilidad)


    def jugar(self, cliente, predecir=False):
        apuesta = cliente.apuesta(self)
...
class Instalacion(Building):
    ...
    def personal_activo(self):
        print(sum([i for i in self.personal if i.activo]))
